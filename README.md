...
| `/ref` (або `/referral`) | партнерська програма (+7 днів підписки за кожного запрошеного) |
...
| `/feedback` (або `/idea`, `/suggest`) | надіслати ідею/побажання щодо покращення (+3 дні безкоштовної підписки як бонус) |
